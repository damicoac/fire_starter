...
func init() {
	RegisterModule("google_dorking_for_apis", func(payload map[string]any, onLog func(string)) (ExecutableModule, error) {

		target := PayloadString(payload, "url", "http://127.0.0.1")
		onLog(fmt.Sprintf("Generating Google dork API queries for: %s", target))

		dorker := NewGoogleDorkingForAPIs(target)

		return ModuleWrapper{
			Module: dorker,
			ExecuteFunc: func(ctx context.Context) (any, error) {
				return dorker.Generate(), nil
			},
		}, nil
	})
}
